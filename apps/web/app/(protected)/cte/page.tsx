...
interface CteConfig {
  ...
  certificado: CteCertificado | null;
  controle: CteControle | null;
}
...
const INTERVALO_OPTIONS = [
  ...
  { label: '1 hora', value: 60 }, { label: '2 horas', value: 120 },
  { label: '4 horas', value: 240 }, { label: '6 horas', value: 360 },
  { label: '12 horas', value: 720 }, { label: '24 horas', value: 1440 },
];

const CSTAT_INFO: Record<string, { descricao: string; tipo: 'ok' | 'info' | 'warn' | 'err' }> = {
  '108': { descricao: 'Serviço paralisado momentaneamente', tipo: 'warn' },
  '109': { descricao: 'Serviço paralisado sem previsão', tipo: 'err' },
  '135': { descricao: 'Evento registrado', tipo: 'ok' },
  '136': { descricao: 'Evento vinculado', tipo: 'ok' },
  ...
  '573': { descricao: 'Evento já registrado (duplicidade)', tipo: 'info' },
...
function pendentesNsu(c: CteControle | null) {
  if (!c) return 0;
...
function ConfigDrawer({
  ...
  const [lotes, setLotes] = useState<CteLote[]>([]);
  ...
  const carregarLotes = useCallback(async (p: number) => {
    ...
    try {
      const res = await api.get(`/cte/lotes?configId=${config.id}&limit=${LIMIT}&page=${p}`);
      const data = res.data as { data: CteLote[]; meta: { total: number } };
...
function NovaConfigModal({ empresas, certificados, onClose, onSave, saving }: Readonly<{
  ...
  return (
    <Modal isOpen onClose={onClose} title="Nova Configuração CT-e" size="2xl">
      <div className="space-y-4">
        ...
        {/* Certificado */}
        <div>
          <label htmlFor="cte-cert" className="block text-xs font-medium text-muted-foreground mb-1.5">
          ...
          {certificados.length === 0 ? (
            ...
            <>
              <select id="cte-cert" title="Certificado A1" value={form.certificadoId}
        ...
        <div>
          <label htmlFor="cte-amb" className="block text-xs font-medium text-muted-foreground mb-1.5">
            Ambiente <span className="text-destructive">*</span>
          </label>
          <select id="cte-amb" title="Ambiente" value={form.tpAmb}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label htmlFor="cte-horario" className="block text-xs font-medium text-muted-foreground mb-1.5">
              Horário de início <span className="text-destructive">*</span>
            </label>
            <input id="cte-horario" type="time" value={form.horarioCaptura}
              onChange={(e) => { set('horarioCaptura', e.target.value); }}
              className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring" />
          </div>
          <div>
            <label htmlFor="cte-intervalo" className="block text-xs font-medium text-muted-foreground mb-1.5">
              Periodicidade <span className="text-destructive">*</span>
            </label>
            <select id="cte-intervalo" title="Periodicidade" value={form.intervaloMinutos}
...
export default function CtePage() {
  const [configs, setConfigs] = useState<CteConfig[]>([]);
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      setConfigs(statusRes.data as CteConfig[]);
      ...
      toastError('Erro ao carregar configurações CT-e');
    } finally {
      setLoading(false);
    }
  }, [toastError]);
  ...
  const handleToggle = useCallback(async (configId: string) => {
    try {
      const res = await api.post(`/cte/${configId}/toggle`);
  ...
  const handleSync = useCallback(async (configId: string) => {
    ...
    try {
      await api.post(`/cte/sincronizar/${configId}`);
  ...
  const handleDelete = useCallback(async (configId: string) => {
    try {
      await api.delete(`/cte/${configId}`);
      success('Configuração CT-e excluída com sucesso');
      setSelectedId(null);
      setConfigs((prev) => prev.filter((c) => c.id !== configId));
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message;
      toastError(msg ?? 'Erro ao excluir configuração CT-e');
      throw err;
    }
  }, [success, toastError]);
  ...
  const handleSave = useCallback(async (form: FormState) => {
    setSaving(true);
    try {
      const empresa = empresas.find((e) => e.cnpj === form.cnpj);
      await api.post('/cte/configurar', {
      ...
      setShowModal(false);
      success('Monitoramento CT-e configurado com sucesso');
      await carregar();
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message;
      toastError(msg ?? 'Erro ao salvar configuração');
    } finally { setSaving(false); }
  }, [empresas, carregar, success, toastError]);
    ...
    <div className="flex flex-col gap-4 flex-1 min-h-0 h-full overflow-y-auto pb-4">
      ...
      <div className="bg-card rounded-lg border border-border shadow-sm overflow-hidden flex-1 min-h-0 flex flex-col">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between shrink-0">
          <div>
            <p className="font-semibold text-foreground">Configurações CT-e</p>
        ...
        <div className="flex-1 min-h-0 overflow-auto">
          <DataTable<CteConfig>
            ...
            emptyMessage={busca || filtroStatus !== 'todos' ? 'Nenhuma configuração encontrada.' : 'Nenhuma configuração CT-e. Clique em "Nova Configuração" para começar.'}
            onRowClick={(row) => {
