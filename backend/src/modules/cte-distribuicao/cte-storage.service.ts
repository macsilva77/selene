...
/**
 * Serviço de armazenamento de XMLs de CT-e no Google Cloud Storage.
 *
 * Quando GCS_BUCKET_NAME (ou GCS_BUCKET) não está configurado (ambiente local
 * sem GCS), o serviço opera em modo desabilitado — `resolverXml` usa o fallback
 * do campo `xmlOriginal` no banco, sem lançar erro.
 *
 ...
 */
@Injectable()
export class CteStorageService {
  private readonly logger = new Logger(CteStorageService.name);
  ...
  /** Caminho canônico do XML no bucket: cte/{tenantId}/{cnpj}/{nsu}.xml */
  xmlPath(tenantId: string, cnpj: string, nsu: string): string {
    return `cte/${tenantId}/${cnpj}/${nsu}.xml`;
  }
  ...
  private assertEnabled(): void {
    if (!this.isEnabled) {
      throw new Error('CteStorageService: GCS_BUCKET_NAME não configurado');
    }
  }
}
