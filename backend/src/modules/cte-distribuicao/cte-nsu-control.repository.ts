 ...
 *  - Controle de NSU (criar/atualizar CteNsuControle)
 *  - Lock distribuído atômico via SQL (evitar consultas concorrentes ao mesmo CNPJ)
 *  - Auditoria (inserir registros em CteAuditoria)
 */
@Injectable()
export class CteNsuControlRepository {
  private readonly logger = new Logger(CteNsuControlRepository.name);
  private readonly processId = `${hostname()}:${process.pid}`;
  private readonly lockTimeoutMs = CTE_WORKER_DEFAULTS.lockTimeoutSegundos * 1000;
  ...
  async obterOuCriarControle(configId: string, tenantId: string, cnpj: string) {
    const existente = await this.prisma.cteNsuControle.findUnique({
      where: { configId },
    });
    if (existente) return existente;

    return this.prisma.cteNsuControle.create({
      data: {
        configId,
        tenantId,
        cnpj,
  ...
  async liberarLockEAtualizarNsu(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      const controle = await tx.cteNsuControle.findUnique({
        where: { id: controleId },
        select: { lockId: true },
      });
      ...
      await tx.cteNsuControle.update({
        where: { id: controleId },
        data: {
          ultimoNsu,
          maxNsu,
  ...
  /**
   * Zera o NSU e libera o cooldown — permite recuperação completa dos 3 meses
  ...
  async resetarNsu(controleId: string): Promise<void> {
    ...
    this.logger.log(`Controle ${controleId}: NSU zerado, cooldown liberado — recuperação dos 3 meses iniciará no próximo ciclo`);
  }
