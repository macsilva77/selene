...
/**
 * Carrega o certificado PEM descriptografado do banco para uso em mTLS com a
 * SEFAZ do CT-e. O mesmo certificado (CertificadoDigital) usado na NF-e serve
 * para o CT-e — aqui apenas resolvemos a partir da CteConfig.
 *
 * Resultados são cacheados por 10 minutos para reduzir operações de crypto.
 */
@Injectable()
export class CteCertLoaderService {
  private readonly logger = new Logger(CteCertLoaderService.name);
  ...
  async loadCert(tenantId: string, configId: string): Promise<{ pemCert: string; pemKey: string }> {
    ...
    // Filtra por tenantId (defesa em profundidade): um configId de outro tenant
    // nunca deve descriptografar a chave privada A1/mTLS deste tenant.
    const config = await this.prisma.cteConfig.findFirstOrThrow({
