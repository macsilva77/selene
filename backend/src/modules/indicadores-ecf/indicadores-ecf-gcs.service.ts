...
@Injectable()
export class IndicadoresEcfGcsService implements OnModuleInit {
  private readonly logger = new Logger(IndicadoresEcfGcsService.name);
  ...
  onModuleInit(): void {
    ...
    this.logger.log('IndicadoresEcfGcsService: GCS inicializado');
  }
  ...
  async downloadFromUri(gcsUri: string): Promise<Buffer> {
    ...
    this.logger.debug(`Baixando ECF: bucket=${bucket} path=${filePath}`);
